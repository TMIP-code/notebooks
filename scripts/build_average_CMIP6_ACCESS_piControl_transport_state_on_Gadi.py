...
print("Time window = last ", num_years, " years")
...
def time_window_strings(year_end, num_years, time_type = datetime.datetime):
    """
    return strings for start_time and end_time
    """
    # start_time is first second of year_start
    start_time = time_type(year_end - num_years + 1, 1, 1, 0, 0, 0)
    # end_time is last second of last_year
    end_time = time_type(year_end, 12, 31, 23, 59, 59)
...
if __name__ == '__main__':
    ...
    for member in sorted_members:
        ...
        year_end = umo_datadask["time"].max('time').values.item().year
        start_time, end_time = time_window_strings(year_end, num_years)
